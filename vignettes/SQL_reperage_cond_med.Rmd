...
author: "Auteur: Guillaume Boucher (développeur); Ahmed Ghachem (réviseur)"
...
`SQL_reperage_cond_med` est une fonction qui permet de reperer une condition médicale basé sur les codes de diagnostic cim-9 et cim-10 dans SMOD, MEDECHO-diag, MEDECHO-sej et BDCU. Elle permet de reperer la condition médicale sans confirmation de diagnostic `nDx=0` ou avec confirmation de diagnostic en utilisant l'argument `nDx=n`. Si l'argument `nDx` est différent de 0, `SQL_reperage_cond_med` utilise l'algorithme décrit ci-dessous.   
...
```{r}
debut_cohort = "2020-01-01", fin_cohort = "2020-12-31"
```
