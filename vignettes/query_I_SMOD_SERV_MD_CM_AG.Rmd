...
1) extraire les diagnostics pour la création d'une cohorte <br/>
2) extraire les dates de visites
3) calculer le nombre d'acte par bénéficiaire entre la date de début et de la date de fin d'une étude <br/>
4) extraire les codes d'actes, ainsi que les caractéristiques de bénéficiaire et de l'établissement de soins <br/>

Elle est appellée par:
- la fonction [SQL_reperage_cond_med](SQL_reperage_cond_med.html) pour l'extraction des diagnostics afin de créer une cohorte
- la fonction [SQL_CodeActe](SQL_CodeActe.html) pour l'extraction des actes 
- ...
...
Indiquant si on veut extraire des diagnostics "extraction_Dx", ou calculer le nombre d'acte "calcul_Nb_acte", ou extraire des codes d'actes "extraction_acte".
```{r}
query="extraction_Dx"
query="date_visite"
...
Un vecteur indiquant les codes de diagnostics *CIM9* et/ou *CIM10* à extraire.
```{r}
...
Un vecteur indiquant les codes d'acte que vous aimeriez extraire.
```{r, eval=FALSE}
...
query_SQL_CodeActe.where_SMOD_COD_SPEC(omni_spec),# query_SQL_CodeActe.where_SMOD_COD_SPEC est une fonction qui retourne le code sql : AND BD_SMOD.SMOD_COD_SPEC IS NULL ou AND BD_SMOD.SMOD_COD_SPEC IS Not NULL
query_SQL_CodeActe.where_ETAB_COD_CATG_ETAB_EI(catg_etab),# query_SQL_CodeActe.where_ETAB_COD_CATG_ETAB_EI est une fonction qui retourne le code sql : AND (ETAB_COD_CATG_ETAB_EI IN ('54X', '55X', '57X', '9X2', '8X5', '4X1','0X1') OR BD_SMOD.SMOD_NO_ETAB_USUEL IS NULL), si catg_etab="ambulatoire"
...
AND BD_SMOD.SMOD_DAT_SERV BETWEEN '",debut,"' AND '",fin,"'
query_SQL_CodeActe.where_SMOD_COD_SPEC(omni_spec),# query_SQL_CodeActe.where_SMOD_COD_SPEC est une fonction qui retourne le code sql : AND BD_SMOD.SMOD_COD_SPEC IS NULL ou AND BD_SMOD.SMOD_COD_SPEC IS Not NULL
query_SQL_CodeActe.where_SMOD_COD_ACTE (CodeActe),# query_SQL_CodeActe.where_SMOD_COD_ACTE est une fonction qui retourne le code sql : AND BD_SMOD.SMOD_COD_ACTE IN (",qu(CodeActe),") si CodeActe is not null
query_SQL_CodeActe.where_ETAB_COD_CATG_ETAB_EI(catg_etab),# query_SQL_CodeActe.where_ETAB_COD_CATG_ETAB_EI est une fonction qui retourne le code sql : AND (ETAB_COD_CATG_ETAB_EI IN ('54X', '55X', '57X', '9X2', '8X5', '4X1','0X1') OR BD_SMOD.SMOD_NO_ETAB_USUEL IS NULL), si catg_etab="ambulatoire"
