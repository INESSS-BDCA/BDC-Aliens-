...
2) calculer le nombre d'acte par bénéficiaire entre la date de début et de fin d'une étude <br/>
3) extraire les codes d'actes, avec les caractéristiques de bénéficiaire et de l'établissement de soins <br/>

Elle est appellée par:
- la fonction [SQL_reperage_cond_med](SQL_reperage_cond_med.html)
- la fonction [SQL_CodeActe](SQL_CodeActe.html)
...
query_SQL_CodeActe.where_SMOD_COD_SPEC(omni_spec), # query_SQL_CodeActe.where_SMOD_COD_SPEC est une fonction qui imprime le code sql : AND BD_SMOD.SMOD_COD_SPEC IS NULL ou AND BD_SMOD.SMOD_COD_SPEC IS Not NULL
query_SQL_CodeActe.where_SMOD_COD_ACTE (CodeActe), # query_SQL_CodeActe.where_SMOD_COD_ACTE est une fonction qui imprime le code sql : AND BD_SMOD.SMOD_COD_ACTE IN (",qu(CodeActe),") si CodeActe is not null
