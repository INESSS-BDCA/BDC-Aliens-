...
La fonction `SQL_CodeActe` est une `child_function` qui permet d'extraire des actes entre une date de début et de fin d'une étude. Elle appelle plusieurs autres fonctions `parent_function` génériques telles que: 
[SQL_reperage_cond_med](SQL_reperage_cond_med.html),<br/> 
[query_SQL_CodeActe](uery_SQL_CodeActe.html),<br/>
...
2) l'extraction des actes pour cette cohorte via la fonction `query_SQL_CodeActe`<br/>
3) l'ajout des caractéristiques des benéficiaires ainsi que l'établissement de soins. Les caractéristiques des bénéficiaires et de l'établissement de soins pour les personnes ayant eu un code d'acte à l'intérieur de la période de l'étude sont extraits via la fonction `query_SQL_CodeActe`, alors que pour ceux qui n'ont pas eu d'acte sont extraits via la fonction `query_V_FICH_ID_BEN_CM`.    
...
Extraction des actes:
 - Temps d'exécution (15.18 secs)
Ajout des caractéristiques des bénéficiaires qui n'ont pas eu d'acte dans la période d'étude:
 - Temps d'exécution (5.27 mins)
Préparation de la base de données finale:
 - Temps d'exécution (9.36 secs)
```
...
[SQL_reperage_cond_med](SQL_reperage_cond_med.html)<br/> 
[query_SQL_CodeActe](query_SQL_CodeActe.html)<br/>
[query_V_FICH_ID_BEN_CM](query_V_FICH_ID_BEN_CM.html)<br/>
