#' @title Code SQL: Extraction de variables pertinentes  dans BDCU
#'
#' @description Générateur de code SQL pour l'extraction de variables pertinentes de les vue `V_EPISO_SOIN_DURG_CM`, `V_OCCU_CIVIE_DURG_CM`, `V_CNSUL_DURG_CM`.

#' @param query indique si on veut effectuer une extraction brute des vaiables pertinentes dans BDCU `V_EPISO_SOIN_DURG_CM`, `V_OCCU_CIVIE_DURG_CM`, `V_CNSUL_DURG_CM`.
...
query_V_EPISO_SOIN_DURG_CM_AG <- function(query,debut_periode, fin_periode,debut_cohort,fin_cohort, diagn, date_dx_var,benef_adr,date_adr) {
  switch (query,
"extraction_BDCU_EPISO_SOIN_DURG"={
            return(paste0(
 "SELECT
-- vue: RES_SSS.V_EPISO_SOIN_DURG_CM
BD_BDCU.SURG_NO_INDIV_BEN_BANLS AS ID,
CAST(BD_BDCU.SURG_NO_EPISO_SOIN_DURG AS CHAR(18)),
--CAST(BD_BDCU.SURG_NO_EPISO_SOIN_DURG AS INT) AS Num_SURG_NO_EPISO_SOIN_DURG,-- Numéro d'épisode de soins - Départements d'urgence
...
"extraction_BDCU_OCCU_CIVIE_DURG"={
            return(paste0("
SELECT
BD_OCCUCIVIE.SURG_NO_INDIV_BEN_BANLS AS ID,--	Numéro d'individu de la personne assurée
...
Cast(BD_OCCUCIVIE.SURG_DHD_OCCU_CIVIE_DURG AS DATE) AS DD_Occup_civ, -- Date début d'occupation de civière
BD_OCCUCIVIE.SURG_DHF_OCCU_CIVIE_DURG,--	Date et heure de fin_cohort d'occupation de civière
Cast(BD_OCCUCIVIE.SURG_DHF_OCCU_CIVIE_DURG AS DATE) AS DF_Occup_civ, -- Date début d'occupation de civière
BD_OCCUCIVIE.SURG_CATG_CIVIE_DURG --	Catégorie de civière en département d'urgence
FROM RES_SSS.V_OCCU_CIVIE_DURG_CM AS BD_OCCUCIVIE
ORDER BY 1;"))},
"extraction_BDCU_CNSUL_DURG"={
            return(paste0("
SELECT
BD_CNSUL.SURG_NO_INDIV_BEN_BANLS AS ID,--	Numéro d'individu de la personne assurée
...
Cast(BD_CNSUL.SURG_DH_DEM_CNSUL_DURG AS DATE) AS DD_DEM_CNSUL, -- Date demande consult
BD_CNSUL.SURG_DH_REAL_CNSUL_DURG, --	Date et heure de réalisation de consultation
Cast(BD_CNSUL.SURG_DH_REAL_CNSUL_DURG AS DATE) AS DD_REAL_CNSUL -- Date realisation consult
FROM RES_SSS.V_CNSUL_DURG_CM AS BD_CNSUL
ORDER BY 1;"))},
"extraction_Dx"={
            return(paste0(
            "SELECT
...
query_V_EPISO_SOIN_DURG_CM.diagn  <- function(diagn) {
  if (!is.null(diagn)){
    ...
    return(paste0(
      indent(),
      "and (BD_BDCU.SURG_COD_DIAGN_MDCAL_CLINQ like any (",qu(cim10),")\n",
      indent(),
      "or\n",
      indent(),
      "BD_BDCU.SURG_COD_DIAGN_MDCAL_CLINQ like any (",qu(cim9),"))\n"
    ))
  }
    ...
    return(paste0(
      indent(),
      "and BD_BDCU.SURG_COD_DIAGN_MDCAL_CLINQ like any (",qu(cim9),")\n"
    ))
  }
    else if (!length(cim9) && length(cim10)) {
    # CIM10 seulement
    ...
    stop("query_V_EPISO_SOIN_DURG_CM.diagn : diagn est vide.")
  }
  }
  else if (is.null(diagn)){
    return(paste0(
      "/*and BD_BDCU.SURG_COD_DIAGN_MDCAL_CLINQ like any ()*/\n"))
  }
}
...
#' @encoding UTF-8
#' @keywords internal
query_V_EPISO_SOIN_DURG_CM.where_I_BENF_ADR_CM<- function(benef_adr,debut_periode,fin_periode,date_adr) {
  switch(benef_adr,
         "date_fixe" = {
           return(paste0(
             indent(),
