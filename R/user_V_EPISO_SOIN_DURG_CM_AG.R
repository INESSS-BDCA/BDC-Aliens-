...
#'                       task=c("extraction_BDCU_EPISO_SOIN_DURG"),
  ...
  switch (task,
          "extraction_BDCU_EPISO_SOIN_DURG"={
            ...
            if(is.null(cohort) && !is.null(Dx_table)){
              ...
              # Extraction d'acte ###
              if (verbose) {
                cat("Étape 2: Extraction des variables BDCU: EPISO SOIN pour la cohorte crée à l'étape 1:\n")
              }

              t1 <- Sys.time()

              DT<-data.table::as.data.table(odbc::dbGetQuery(conn=conn,
                                                             statement=query_V_EPISO_SOIN_DURG_CM_AG(query="extraction_BDCU_EPISO_SOIN_DURG",
              ...
              if (verbose) {
                if(is.null(cohort)){
                  cat("Étape 1: Extraction des variables BDCU: EPISO SOIN pour les bénéficiaires qui ont eu recours aux services d'urgence à l'intérieur de la période de l'étude:\n")
                }
                else{
                  cat("Étape 1: Extraction des variables BDCU: EPISO SOIN pour la cohorte fournie par l'analyste:\n")
                }
              }
          ...
          "extraction_BDCU_OCCU_CIVIE_DURG"={
            ...
            if(is.null(cohort) && !is.null(Dx_table)){
              ...
              # Extraction d'acte ###
              if (verbose) {
                cat("Étape 2: Extraction des variables BDCU: OCCU CIVIE pour la cohorte crée à l'étape 1:\n")
              }

              t1 <- Sys.time()

              DT<-data.table::as.data.table(odbc::dbGetQuery(conn=conn,
                                                             statement=query_V_EPISO_SOIN_DURG_CM_AG(query="extraction_BDCU_OCCU_CIVIE_DURG",
                                                                                                     debut_periode=debut_periode,
                                                                                                     fin_periode=fin_periode,
                                                                                                     debut_cohort=debut_cohort,
                                                                                                     fin_cohort=fin_cohort,
              ...
              if (verbose) {
                if(is.null(cohort)){
                  cat("Étape 1: Extraction des variables BDCU: OCCU CIVIE pour les bénéficiaires qui ont eu recours aux services d'urgence à l'intérieur de la période de l'étude:\n")
                }
                else{
                  cat("Étape 1: Extraction des variables BDCU: OCCU CIVIE pour la cohorte fournie par l'analyste:\n")
                }
              }
            ...
            if(is.null(cohort) && keep_all==FALSE){ #
              DT_final<-DT_final %>% filter(!is.na(DD_Occup_civ ))
            }
            else if(length(cohort) > 0 && keep_all==FALSE){
              cohort<-cohort %>% data.frame() %>% rename(ID=".") %>% left_join(DT_final,by="ID") %>% filter(!is.na(DD_Occup_civ))
              DT_final<-cohort
            }
            else if (length(cohort) > 0 && keep_all== TRUE){
              cohort<-cohort %>% data.frame() %>% rename(ID=".") %>% left_join(DT_final,by="ID")
          ...
          "extraction_BDCU_CNSUL_DURG"={
            ...
            if(is.null(cohort) && !is.null(Dx_table)){
              ...
              # Extraction d'acte ###
              if (verbose) {
                cat("Étape 2: Extraction des variables BDCU: CNSUL pour la cohorte crée à l'étape 1:\n")
              }

              t1 <- Sys.time()

              DT<-data.table::as.data.table(odbc::dbGetQuery(conn=conn,
                                                             statement=query_V_EPISO_SOIN_DURG_CM_AG(query="extraction_BDCU_CNSUL_DURG",
                                                                                                     debut_periode=debut_periode,
                                                                                                     fin_periode=fin_periode,
                                                                                                     debut_cohort=debut_cohort,
                                                                                                     fin_cohort=fin_cohort,
              ...
              if (verbose) {
                if(is.null(cohort)){
                  cat("Étape 1: Extraction des variables BDCU: CNSUL pour les bénéficiaires qui ont eu recours aux services d'urgence à l'intérieur de la période de l'étude:\n")
                }
                else{
                  cat("Étape 1: Extraction des variables BDCU: CNSUL pour la cohorte fournie par l'analyste:\n")
                }
              }
            ...
            if(is.null(cohort) && keep_all==FALSE){ #
              DT_final<-DT_final %>% filter(!is.na(DD_DEM_CNSUL ))
            }
            else if(length(cohort) > 0 && keep_all==FALSE){
              cohort<-cohort %>% data.frame() %>% rename(ID=".") %>% left_join(DT_final,by="ID") %>% filter(!is.na(DD_DEM_CNSUL))
              DT_final<-cohort
            }
            else if (length(cohort) > 0 && keep_all== TRUE){
              cohort<-cohort %>% data.frame() %>% rename(ID=".") %>% left_join(DT_final,by="ID")
