...
#'
#' @description  `query_I_SMOD_SERV_MD_CM_AG` est une fonction multi-tâches. Elle utilise la vue `I_SMOD_SERV_MD_CM` pour générer un code SQL qui permet :
#' 1) d'extraire les diagnostics pour créer par exemple une cohorte.
#' 2) de calculer le nombre d'acte par bénéficiaire entre la date de début et de fin d'une étude.
#' 3) d'extraire les codes d'actes, avec les caractéristiques de bénéficiaire et de l'établissement de soins.
#'
...
query_I_SMOD_SERV_MD_CM_AG <- function(query,debut, fin, diagn,omni_spec,CodeActe,catg_etab) {
  switch (query,
            ...
            return(paste0(
              "SELECT
                ...
                FROM(
                  SELECT DISTINCT
                    BD_SMOD.SMOD_NO_INDIV_BEN_BANLS AS ID,
                    BD_SMOD.SMOD_DAT_SERV AS Dat_visit,
          ...
          "calcul_Nb_acte"={
            return(paste0(
              "SELECT
            SMOD_NO_INDIV_BEN_BANLS AS ID,
            Sum(acte) AS nb_actes
            FROM (
            SELECT DISTINCT
            BD_SMOD.SMOD_NO_INDIV_BEN_BANLS,
            BD_SMOD.SMOD_DAT_SERV,
            BD_SMOD.DISP_NO_SEQ_DISP_BANLS,
            1 AS acte
            FROM Prod.I_SMOD_SERV_MD_CM AS BD_SMOD
            WHERE BD_SMOD.SMOD_COD_STA_DECIS IN ('PAY')
...
#' @description Section `where` du code SQL où on demande les actes.
#' @encoding UTF-8
#' @keywords internal
...
#' @description Section `where` du code SQL où on demande les visites en ambulatoire.
#' @encoding UTF-8
#' @keywords internal
